...
# docker build -t app_test_api .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name app_test_api app_test_api
...
# blocks the `rails` user from writing sibling paths like .local/state.
